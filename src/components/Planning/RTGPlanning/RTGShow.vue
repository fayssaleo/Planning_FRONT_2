<template>
  <v-row class="saveButtonBar" style="height: 44px !important; color: black" v-if="getJustCreated == false">
    <v-col cols="8" style="position: relative">
      <span>Click on <span class="confirmIconText">CONFIRM</span> button to confirm
        this planning!</span>
    </v-col>
    <v-col cols="4" style="position: relative">
      <v-btn density="default" class="saveButton3" @click="openCancelMDialog">CANCEL</v-btn>
      <v-btn density="default" class="saveButton" @click="openSaveDialog">CONFIRM</v-btn>
    </v-col>
  </v-row>
  <v-row style="width: 90%; margin: 0 auto">
    <v-col class="header" cols="12" style="position: relative">
      <div class="buttonsHolder">
        <div class="button" :class="editMode ? 'buttonOn2' : ''" @click="closePlanning">
        ...
        <thead>
          <tr v-if="editMode" class="addDriverButtonClickableCase">
            ...
            <th v-for="(outs, index) in OutToBeInserted" :key="index" style="position: relative; margin: 0; padding: 0">
              ...
              ]" style="padding-top: 16px; z-index: 1" @click="outs.checked = !outs.checked">
                OUT ({{ outs.list.length }})
                <span v-if="outs.list.length > 0" class="equipemntOuts">
                  <span @click.stop="" v-for="(outItem, index2) in outs.list" :key="index + index2"
                    class="equipmentOutOne">
        ...
        <template v-slot:default="{ isActive }">
          <v-card class="dialogHeader">
            <div class="sticky-header">
              <v-toolbar flat>
                <v-toolbar-title>
                  RTG SHIFT PLANNING - {{ actualShift.name }} (Date:
                  {{ selectedDate.toLocaleDateString() }} -
                  {{ getShiftPeriode }}):
                </v-toolbar-title>
                <v-btn class="cancelButton" text="CANCEL" @click="isActive.value = false"></v-btn>
                <v-btn density="default" class="saveButton2" @click="ifCheckEditedPlanning?openClickApply():openSaveCONFIRMDialog()">APPLY</v-btn>
              </v-toolbar>
            </div>
    ...
    <div class="text-center pa-4">
      <v-dialog v-model="cancelDialog" max-width="500" persistent>
        <v-card class="confirmDialogInterface" prepend-icon="mdi-trash-can-outline"
          text="Are you sure you want to cancel this planning?" title="Confirm saving">
          <template v-slot:actions>
            <v-spacer></v-spacer>
  ...
  methods: {
    ...
    clickApply(){
      if(this.reloadPlanning){
        ...
        console.log("paylodPlanning",paylodPlanning)
        console.log("this.tempPlanning_forShow",this.tempPlanning_forShow)
        console.log("this.getPlanningWithTheBoxes[0]",this.getPlanningWithTheBoxes[0])
      ...
      this.setOutToBeInserted(this.ThisOutToBeInserted);
    ...
    clickFinish() {
      ...
      if (this.reloadPlanning) {
        ...
        if (this.driversInPlanningWithCheck.length > this.equipmentsInPlanningWithCheck.length) {
          ...
          console.log("this.ShiftIndex", this.ShiftIndex);
          console.log("this.selectedPlanning", this.selectedPlanning);
          ...
          console.log("dedededede",this.ThisOutToBeInserted)
          }
          this.$emit(
            "rtgFullPlanning",
            this.driversInPlanningWithCheck,
        ...
        console.log("tempPlanning_2",[...tempPlanning_2])
        ...
        for (
          ...
          if (index < equipmentInPlanning_now.length) {
            console.log("na3Am : ",equipmentInPlanning_now[index])
    ...
    switchEquipmentFromRealPlanning(tempPlanning,tempPlanning_2, equipmentOld, equipmentNew) {
      ...
      console.log("tempPlanning2 :",tempPlanning2);
...
.sticky-header {
  ...
  top: 0;
  z-index: 5;
  /* Ensures the header stays above the scrolled content */
